...
ALIO API는 해외 IP를 차단하므로 GitHub Actions(미국 서버)에서 직접 수집할 수 없습니다. 그래서 두 갈래로 나눕니다.

- **GitHub Actions (자동, 하루 4회)**: 잡코리아·사람인·자소설닷컴을 수집하고, 저장소에 커밋된 `data/alio_snapshot.json`을 불러와 합친 뒤 GitHub Pages에 배포합니다 (`ALIO_PUBLIC_ENABLED=0`).
- **내 PC (수동, 1커맨드)**: 한국 IP라 ALIO 수집이 가능합니다. 아래 한 줄이면 전체 수집 → 정리 → 스냅샷 갱신 → 커밋/푸시까지 끝나고, 푸시가 CI를 트리거해 웹이 갱신됩니다.

```powershell
$env:PYTHONPATH='src'
python -m job_radar publish        # ALIO 최신분을 웹에 반영
python -m job_radar publish --no-push  # 수집·스냅샷만, git 생략
```

PC가 꺼져 있어도 플랫폼 3종은 웹에서 계속 갱신되고, ALIO는 마지막 publish 시점 기준으로 유지됩니다.

- 수동 갱신은 GitHub Actions 화면의 `workflow_dispatch`로도 실행할 수 있습니다.
